...
def criar_mediainfo(caminho_arquivo, caminho_saida=None):
    ...
    Retorna:
    str: Caminho do arquivo .nfo gerado ou None em caso de erro
    """
    try:
...
def obter_idiomas_legendas(caminho_arquivo):
    ...
    Retorna:
    list: Lista de idiomas das legendas ou lista vazia em caso de erro
    """
    try:
        # Verifica se o arquivo existe
        if not os.path.exists(caminho_arquivo):
        ...
        # Comando para extrair informações com mediainfo
        comando_mediainfo = [
            "mediainfo",
            caminho_arquivo
        ]
        ...
        # Processa a saída para encontrar os idiomas das legendas
        linhas = resultado.stdout.splitlines()
        idiomas = []
        ...
        for linha in linhas:
            if linha.startswith("Text"):
                ...
                em_secao_texto = False  # Assume uma legenda por seção até a próxima "Text"
